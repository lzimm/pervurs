...
from pervurs import import_comp
...
class ModelField(Field):
    ...
    def _prep(self, ref, counterpart = True):
        if self._model is None:
            self._model = import_comp('pervurs.models', self.model)
        ...
        if self._counterpart is None and counterpart:
            ...
            for field in self._model._fields:
                if getattr(self._model._fields[field], '_model', True) is None:
                    self._model._fields[field]._model = import_comp('pervurs.models', self._model._fields[field].model)
